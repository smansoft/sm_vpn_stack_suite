    ...
    *qname = 0;
    ...
    while (len) {
        if (len & 0xC0) {
            ...
            len = *(data + ptr);
            log_android(ANDROID_LOG_DEBUG, __FUNCTION__,"DNS qname compression ptr %d len %d", ptr, len);
            if (!c) {
                c = 1;
                off += 2;
            }
    ...
    if (len > 0 || noff == 0) {
        log_android(ANDROID_LOG_ERROR, __FUNCTION__,"DNS qname invalid len %d noff %d", len, noff);
        return -1;
    }

    *(qname + noff - 1) = 0;
    log_android(ANDROID_LOG_DEBUG, __FUNCTION__,"qname %s", qname);
...
void parse_dns_response(const struct arguments *args, const struct udp_session *u,
    ...
    if (*datalen < sizeof(struct dns_header) + 1) {
        log_android(ANDROID_LOG_WARN, __FUNCTION__,"DNS response length %d", *datalen);
        return;
    }
    ...
    if (dns->qr == 1 && dns->opcode == 0 && qcount > 0 && acount > 0) {
        log_android(ANDROID_LOG_DEBUG, __FUNCTION__,"DNS response qcount %d acount %d", qcount, acount);
        if (qcount > 1)
            log_android(ANDROID_LOG_WARN, __FUNCTION__,"DNS response qcount %d acount %d", qcount, acount);
...
int get_dns_query(const struct arguments *args, const struct udp_session *u,
    ...
    if (datalen < sizeof(struct dns_header) + 1) {
        log_android(ANDROID_LOG_WARN, __FUNCTION__, "DNS query length %d", datalen);
        return -1;
    }
    ...
    if (dns->qr == 0 && dns->opcode == 0 && qcount > 0) {
        if (qcount > 1)
            log_android(ANDROID_LOG_WARN, __FUNCTION__, "DNS query qcount %d", qcount);
        ...
        if (off > 0 && off + 4 == datalen) {
            ...
            *qclass = ntohs(*((uint16_t *) (data + off + 2)));
            ...
            log_android(ANDROID_LOG_WARN, __FUNCTION__,"DNS query invalid off %d datalen %d", off, datalen);
    }
