...
int check_dhcp(const struct arguments *args, const struct udp_session *u,
    ...
    log_android(ANDROID_LOG_WARN, __FUNCTION__, "DHCP check");

    if (datalen < sizeof(struct dhcp_packet)) {
        log_android(ANDROID_LOG_WARN, __FUNCTION__,"DHCP packet size %d", datalen);
        return -1;
    }
    ...
    if (ntohl(request->option_format) != DHCP_OPTION_MAGIC_NUMBER) {
        log_android(ANDROID_LOG_WARN, __FUNCTION__,"DHCP invalid magic %x", request->option_format);
        return -1;
    }

    if (request->htype != 1 || request->hlen != 6) {
        log_android(ANDROID_LOG_WARN, __FUNCTION__,"DHCP unknown hardware htype %d hlen %d",
                    request->htype, request->hlen);
        return -1;
    }

    log_android(ANDROID_LOG_WARN, __FUNCTION__,"DHCP opcode", request->opcode);
