...
int check_icmp_session(const struct arguments *args, struct vpn_session *s,
    ...
    if (s->icmp.stop || s->icmp.time + timeout < now) {
        ...
        log_android(ANDROID_LOG_WARN, __FUNCTION__,"ICMP idle %d/%d sec stop %d from %s to %s",
                    now - s->icmp.time, timeout, s->icmp.stop, dest, source);

        if (close(s->socket))
            log_android(ANDROID_LOG_ERROR, __FUNCTION__,"ICMP close %d error %d: %s",
...
void check_icmp_socket(const struct arguments *args, const struct epoll_event *ev) {
    ...
    if (ev->events & EPOLLERR) {
        ...
        if (err < 0)
            log_android(ANDROID_LOG_ERROR, __FUNCTION__,"ICMP getsockopt error %d: %s",
                        errno, strerror(errno));
        else if (serr)
            log_android(ANDROID_LOG_ERROR, __FUNCTION__,"ICMP SO_ERROR %d: %s",
                        serr, strerror(serr));
        ...
        if (ev->events & EPOLLIN) {
            ...
            if (bytes < 0) {
                // Socket error
                log_android(ANDROID_LOG_WARN, __FUNCTION__, "ICMP recv error %d: %s",
                            errno, strerror(errno));
                ...
                log_android(ANDROID_LOG_WARN, __FUNCTION__, "ICMP recv eof");
                s->icmp.stop = 1;
                ...
                if(buffer && bytes > 0) {
                    ...
                    log_android(ANDROID_LOG_DEBUG, __FUNCTION__,"ICMP recv bytes (str) %s", bytes_s);
                    free(bytes_s);
                    bytes_s = to_hex(buffer, bytes);
                    log_android(ANDROID_LOG_DEBUG, __FUNCTION__,"ICMP recv bytes (hex) %s", bytes_s);
                    free(bytes_s);
                }
...
jboolean handle_icmp(const struct arguments *args,
    ...
    if (icmp->icmp_type != ICMP_ECHO) {
        log_android(ANDROID_LOG_WARN, __FUNCTION__,"ICMP type %d code %d from %s to %s not supported",
    ...
    if (cur == NULL) {
        log_android(ANDROID_LOG_INFO, __FUNCTION__,"ICMP new session from %s to %s", source, dest);
        ...
        log_android(ANDROID_LOG_DEBUG, __FUNCTION__,"ICMP socket %d id %x", s->socket, s->icmp.id);
        ...
        if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, s->socket, &s->ev))
            log_android(ANDROID_LOG_ERROR, __FUNCTION__,"epoll add icmp error %d: %s", errno, strerror(errno));
    ...
    if (sendto(cur->socket, icmp, (socklen_t) icmplen, MSG_NOSIGNAL,
        ...
        log_android(ANDROID_LOG_ERROR, __FUNCTION__,"ICMP sendto error %d: %s", errno, strerror(errno));
        if (errno != EINTR && errno != EAGAIN) {
            cur->icmp.stop = 1;
            return 0;
        }
...
int open_icmp_socket(const struct arguments *args, const struct icmp_session *cur) {
    ...
    if (sock < 0) {
        log_android(ANDROID_LOG_ERROR, __FUNCTION__,"ICMP socket error %d: %s", errno, strerror(errno));
        return -1;
    }
...
ssize_t write_icmp(const struct arguments *args, const struct icmp_session *cur,
    ...
    if(data && datalen > 0) {
        ...
        log_android(ANDROID_LOG_DEBUG, __FUNCTION__,"ICMP sending (str) %s", bytes_s);
        free(bytes_s);
        bytes_s = to_hex(data, datalen);
        log_android(ANDROID_LOG_DEBUG, __FUNCTION__,"ICMP sending (hex) %s", bytes_s);
        free(bytes_s);
    }
