...
int check_tun(const struct arguments *args,
    ...
    if (ev->events & EPOLLIN) {
        ...
        if (length < 0) {
            ...
            log_android(ANDROID_LOG_ERROR, __FUNCTION__, "tun %d read error %d: %s",
            ...
            if (length > max_tun_msg) {
                max_tun_msg = length;
                log_android(ANDROID_LOG_WARN, __FUNCTION__, "Maximum tun msg length %d", max_tun_msg);
            }
            ...
            log_android(ANDROID_LOG_ERROR, __FUNCTION__, "tun %d empty read", args->tun);
            return -1;
        }
    }
...
void handle_ip(const struct arguments *args,
    ...
    if (version == 4) {
        if (length < sizeof(struct iphdr)) {
            log_android(ANDROID_LOG_WARN, __FUNCTION__, "IP4 packet too short length %d", length);
            return;
        }
        ...
        if (ip4hdr->frag_off & IP_MF) {
            log_android(ANDROID_LOG_ERROR, __FUNCTION__, "IP fragment offset %u",
        ...
        if (ntohs(ip4hdr->tot_len) != length) {
            log_android(ANDROID_LOG_ERROR, __FUNCTION__,"Invalid length %u header length %u",
        ...
        if (loglevel < ANDROID_LOG_WARN) {
            if (!calc_checksum(0, (uint8_t *) ip4hdr, sizeof(struct iphdr))) {
                log_android(ANDROID_LOG_ERROR, __FUNCTION__, "Invalid IP checksum");
                return;
            }
        }
    } else if (version == 6) {
        if (length < sizeof(struct ip6_hdr)) {
            log_android(ANDROID_LOG_WARN, __FUNCTION__, "IP6 packet too short length %d", length);
            return;
        }
        ...
        if (!is_upper_layer(protocol)) {
            log_android(ANDROID_LOG_WARN, __FUNCTION__, "IP6 extension %d", protocol);
            ...
            if (!is_upper_layer(protocol)) {
                off = 0;
                protocol = ip6hdr->ip6_nxt;
                log_android(ANDROID_LOG_WARN, __FUNCTION__, "IP6 final extension %d", protocol);
            }
        }
        ...
        log_android(ANDROID_LOG_ERROR, __FUNCTION__, "Unknown version %d", version);
        return;
    }
    ...
    if (protocol == IPPROTO_ICMP || protocol == IPPROTO_ICMPV6) {
        if (length - (payload - pkt) < sizeof(struct icmp)) {
            log_android(ANDROID_LOG_WARN, __FUNCTION__, "ICMP packet too short");
            return;
        }
        ...
        if (length - (payload - pkt) < sizeof(struct udphdr)) {
            log_android(ANDROID_LOG_WARN, __FUNCTION__, "UDP packet too short");
            return;
        }
        ...
        if (length - (payload - pkt) < sizeof(struct tcphdr)) {
            log_android(ANDROID_LOG_WARN, __FUNCTION__, "TCP packet too short");
            return;
        }
        ...
        log_android(ANDROID_LOG_WARN, __FUNCTION__, "Unknown protocol %d", protocol);
...
jint get_uid(const int version, const int protocol,
    ...
    char dest[INET6_ADDRSTRLEN + 1];
    inet_ntop(version == 4 ? AF_INET : AF_INET6, daddr, dest, sizeof(dest));
    log_android(ANDROID_LOG_INFO, __FUNCTION__, "get uid v%d p%d %u > %s/%u",
    ...
    if (uid < 0)
        log_android(ANDROID_LOG_ERROR, __FUNCTION__,"uid v%d p%d %u > %s/%u not found",
                    version, protocol, sport, dest, dport);
...
jint get_uid_sub(const int version, const int protocol,
    ...
    FILE *fd = fopen(fn, "r");
    if (fd == NULL) {
        log_android(ANDROID_LOG_ERROR, __FUNCTION__,"fopen %s error %d: %s", fn, errno, strerror(errno));
        return uid;
    }
        ...
        if (i++) {
            *hex = 0;
            ...
            if (fields == 4 && (version == 4 ? strlen(hex) == 8 : strlen(hex) == 32)) {
                ...
                log_android(ANDROID_LOG_ERROR, __FUNCTION__,"Invalid field #%d: %s", fields, line);
        }
    }

    if (fclose(fd))
        log_android(ANDROID_LOG_ERROR, __FUNCTION__,"fclose %s error %d: %s", fn, errno, strerror(errno));
