...
int check_udp_session(const struct arguments *args, struct vpn_session *s,
    ...
    if (s->udp.state == UDP_ACTIVE && s->udp.time + timeout < now) {
        log_android(ANDROID_LOG_WARN, __FUNCTION__, "UDP idle %d/%d sec state %d from %s/%u to %s/%u",
    ...
    if (s->udp.state == UDP_FINISHING) {
        log_android(ANDROID_LOG_INFO, __FUNCTION__,"UDP close from %s/%u to %s/%u socket %d",
                    source, ntohs(s->udp.source), dest, ntohs(s->udp.dest), s->socket);

        if (close(s->socket))
            log_android(ANDROID_LOG_ERROR, __FUNCTION__, "UDP close %d error %d: %s",
...
void check_udp_socket(const struct arguments *args, const struct epoll_event *ev) {
    ...
    if (ev->events & EPOLLERR) {
        ...
        if (err < 0)
            log_android(ANDROID_LOG_ERROR, __FUNCTION__,"UDP getsockopt error %d: %s",
                        errno, strerror(errno));
        else if (serr)
            log_android(ANDROID_LOG_ERROR, __FUNCTION__,"UDP SO_ERROR %d: %s", serr, strerror(serr));
        ...
        if (ev->events & EPOLLIN) {
            ...
            if (bytes < 0) {
                // Socket error
                log_android(ANDROID_LOG_WARN, __FUNCTION__,"UDP recv error %d: %s",
                            errno, strerror(errno));

                if (errno != EINTR && errno != EAGAIN)
                    s->udp.state = UDP_FINISHING;
            } else if (bytes == 0) {
                log_android(ANDROID_LOG_WARN, __FUNCTION__, "UDP recv eof");
                s->udp.state = UDP_FINISHING;
                ...
                log_android(ANDROID_LOG_INFO, __FUNCTION__,
                            "UDP recv bytes %d from %s/%u for tun",
                            bytes, dest, ntohs(s->udp.dest));

                if(buffer && bytes > 0) {
                    char* bytes_s = to_str(buffer, bytes);
                    log_android(ANDROID_LOG_DEBUG, __FUNCTION__,"UDP recv bytes (str) %s", bytes_s);
                    free(bytes_s);
                    bytes_s = to_hex(buffer, bytes);
                    log_android(ANDROID_LOG_DEBUG, __FUNCTION__,"UDP recv bytes (hex) %s", bytes_s);
                    free(bytes_s);
                }
...
jboolean handle_udp(const struct arguments *args,
    ...
    if (cur != NULL && cur->udp.state != UDP_ACTIVE) {
        log_android(ANDROID_LOG_INFO, __FUNCTION__,"UDP ignore session from %s/%u to %s/%u state %d",
    ...
    if (cur == NULL) {
        log_android(ANDROID_LOG_INFO, __FUNCTION__,"UDP new session from %s/%u to %s/%u",
                    source, ntohs(udphdr->source), dest, ntohs(udphdr->dest));
        ...
        log_android(ANDROID_LOG_DEBUG, __FUNCTION__,"UDP socket %d", s->socket);
        ...
        if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, s->socket, &s->ev))
            log_android(ANDROID_LOG_ERROR, __FUNCTION__,"epoll add udp error %d: %s", errno, strerror(errno));
    ...
    if (ntohs(udphdr->dest) == 53) {
        ...
        if (get_dns_query(args, &cur->udp, data, datalen, &qtype, &qclass, qname) >= 0) {
            log_android(ANDROID_LOG_DEBUG, __FUNCTION__,
    ...
    log_android(ANDROID_LOG_INFO, __FUNCTION__,"UDP forward from tun %s/%u to %s/%u data %d",
                source, ntohs(udphdr->source), dest, ntohs(udphdr->dest), datalen);

    if(data && datalen > 0) {
        char* bytes_s = to_str(data, datalen);
        log_android(ANDROID_LOG_INFO, __FUNCTION__,"UDP forward from tun data (str) %s", bytes_s);
        free(bytes_s);
        bytes_s = to_hex(data, datalen);
        log_android(ANDROID_LOG_INFO, __FUNCTION__,"UDP forward from tun data (hex) %s", bytes_s);
        free(bytes_s);
    }
    ...
    if (sendto(cur->socket, data, (socklen_t) datalen, MSG_NOSIGNAL,
        ...
        log_android(ANDROID_LOG_ERROR, __FUNCTION__,"UDP sendto error %d: %s", errno, strerror(errno));
        if (errno != EINTR && errno != EAGAIN) {
...
int open_udp_socket(const struct arguments *args,
    ...
    if (sock < 0) {
        log_android(ANDROID_LOG_ERROR, __FUNCTION__,"UDP socket error %d: %s", errno, strerror(errno));
        return -1;
    }
    ...
    if (cur->version == 4) {
        ...
        if (memcmp(&cur->daddr.ip4, &broadcast4, sizeof(broadcast4)) == 0) {
            log_android(ANDROID_LOG_WARN, __FUNCTION__, "UDP4 broadcast");
            int on = 1;
            if (setsockopt(sock, SOL_SOCKET, SO_BROADCAST, &on, sizeof(on)))
                log_android(ANDROID_LOG_ERROR, __FUNCTION__,"UDP setsockopt SO_BROADCAST error %d: %s",
                            errno, strerror(errno));
        }
    } else {
        // http://man7.org/linux/man-pages/man7/ipv6.7.html
        if (*((uint8_t *) &cur->daddr.ip6) == 0xFF) {
            log_android(ANDROID_LOG_WARN, __FUNCTION__, "UDP6 broadcast");
...
ssize_t write_udp(const struct arguments *args, const struct udp_session *cur,
    ...
    if(buffer && len > 0) {
        ...
        log_android(ANDROID_LOG_DEBUG, __FUNCTION__,"UDP sending to tun data (str) %s", bytes_s);
        free(bytes_s);
        bytes_s = to_hex(buffer, len);
        log_android(ANDROID_LOG_DEBUG, __FUNCTION__,"UDP sending to tun data (hex) %s", bytes_s);
        free(bytes_s);
    }
    ...
    if (res != len) {
        log_android(ANDROID_LOG_ERROR, __FUNCTION__,"write %d/%d", res, len);
        return -1;
    }
